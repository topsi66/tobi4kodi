...
if plugintools.get_setting("use_proxy")=="true":
    MAIN_URL = "https://movie4k.unblocked.pw/"
    SITE_DOMAIN = "movie4k.unblocked.pw"
else:
...
def movies_random(params):
    ...
    plugintools.add_item(action="single_movie", title='Any genre', url=urljoin(MAIN_URL, random_single), extra='random', fanart=FANART , folder=True )
    ...
    for scrapedtitle, scrapedurl in matches:
        ...
        plugintools.add_item(action="single_movie", title=title, url=url, extra='random', fanart=FANART, folder=True )
...
def letters(params):
    ...
    plugintools.add_item(action="letter_pages", title="#", thumbnail="" , plot="", fanart=FANART, url=params.get("url"), folder=True)
    for scrapedurl, letter in matches:
        ...
        plugintools.add_item(action="letter_pages", title=title, fanart=FANART, url=url, extra=extra, folder=True)
...
def letter_pages(params):
    ...
    plugintools.add_item(action="movies_updates", title="Page 1", fanart=FANART, url=params.get("url"), folder=True)
    for scrapedurl, pagenumber in matches:
        url = urljoin(params.get("url"),scrapedurl)
        plugintools.log("movie4k.letters title= Page " + pagenumber + ', scrapedurl=' + scrapedurl + ", url=" + url)
        if params.get("extra")=="tvshows-all":
            plugintools.add_item(action="tvshow_all", title="Page " + pagenumber, fanart=FANART, url=url, folder=True)
        else:
            plugintools.add_item(action="movies_updates", title="Page " + pagenumber, fanart=FANART, url=url, folder=True)
...
def genres(params):
    ...
    for scrapedurl, scrapedtitle in matches:
        ...
        if params.get("extra") == "tvshows-genre":
            plugintools.add_item( action="tvshows_all", title=title, url=url, fanart=FANART, folder=True )
        else:
            plugintools.add_item( action="letter_pages", title=title, url=url, fanart=FANART, folder=True )
...
def read_body_and_headers_cached(url, post=None, headers=[], follow_redirects=False, timeout=None):
...
def html_to_text(string):
    return BeautifulSoup(string, from_encoding='utf-8').getText()
