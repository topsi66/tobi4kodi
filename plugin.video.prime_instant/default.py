...
def listWatchList(url):
    ...
    for i in range(1, len(spl), 1):
        ...
        if "/library/" in url or ("/watchlist/" in url and ("class='prime-meta'" in entry or 'class="prime-logo"' in entry or "class='item-green'" in entry)):
            ...
            if match:
                ...
                title = match[0]
                if showAvailability:
                    match = re.compile('<span class="item-amber">(.+?)</span>', re.DOTALL).findall(entry)
                    if match:
                        title = title + " - " + match[0]
                title = cleanTitle(title)
...
def cleanTitle(title):
    ...
    title = title.replace("\xe4","ä").replace("\xe9","é").replace("\xf6","ö").replace("\xfc","ü").replace("\xc4","Ä").replace("\xd6","Ö").replace("\xdc","Ü").replace("\xdf","ß").strip()
    printable = string.printable.join("éäöüÄÖÜß…")
    title = filter(lambda c: c in printable, title)
    return title
